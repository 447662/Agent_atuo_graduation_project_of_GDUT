...
\subsection{Verilog HDL 与 FPGA 设计}
...
\subsection{AXI Stream、AXI-Lite 与 Wishbone 总线}
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
    \item 搭建 MyHDL 与 Icarus Verilog 环境，运行项目测试脚本。
    \item 保存 \texttt{test\_i2c\_master.py}、\texttt{test\_i2c\_slave.py}、\texttt{test\_i2c\_init.py} 等测试日志。
    \item 对关键事务生成波形截图，展示起始条件、地址阶段、数据阶段、ACK 和停止条件。
    \item 选择目标 FPGA 器件进行综合，记录 LUT、触发器、块 RAM 等资源占用。
    \item 添加一个真实 I2C 外设或仿真存储器的演示工程，验证控制器实际读写流程。
    \item 根据实验结果补充论文第六章，使“验证结果”从结构分析提升为可复现实验结论。
\end{enumerate}
