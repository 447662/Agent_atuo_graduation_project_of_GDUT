...
随着 FPGA 在嵌入式控制、数据采集、工业通信以及片上系统原型验证中的应用不断扩大，外设接口的可复用设计成为数字系统开发中的重要问题。I2C 总线由于引脚数量少、协议结构清晰、外围器件支持广泛，常用于连接 EEPROM、时钟芯片、传感器、电源管理芯片和低速控制外设。在 FPGA 系统中，如果每个项目都重新实现 I2C 通信逻辑，不仅会增加开发周期，也容易在时序控制、应答检测、总线仲裁、时钟拉伸和异常处理等方面引入重复错误。因此，设计一套结构清晰、可复用、便于片上总线集成并配套验证环境的 I2C 接口组件库，具有较强的工程意义和教学参考价值。

本文以 `verilog-i2c-master` 项目为研究对象，围绕其 Verilog I2C 接口组件库的功能结构、模块设计、关键实现逻辑和验证体系展开分析。该项目提供了 I2C 主控制器、I2C 从控制器、AXI-Lite 主机包装层、Wishbone 主机包装层、I2C 到片上总线桥接模块、上电初始化模块、单寄存器从设备示例以及配套的 MyHDL 协同仿真测试平台。本文首先介绍 I2C 总线在 FPGA 外设控制中的应用背景，分析项目面向的功能需求与非功能约束；随后从总体架构角度说明项目采用“核心协议引擎、片上总线包装层、应用示例模块、测试支撑层”的分层组织方式；接着重点分析 `i2c_master.v`、`i2c_slave.v`、`i2c_master_axil.v`、`i2c_slave_axil_master.v`、`i2c_init.v` 等核心模块的功能、接口、数据流和设计思想；最后结合项目测试平台，讨论其验证方式、已有材料能够支持的结果以及后续改进方向。

研究表明，该项目具有较好的模块化、参数化和复用性。其核心模块通过 AXI Stream 风格接口实现控制逻辑与协议时序解耦，并通过 AXI-Lite、Wishbone 等包装层适配常见片上总线系统；测试平台则利用 Python/MyHDL 总线模型与 Icarus Verilog 协同仿真，对主机、从机、初始化流程和桥接场景进行验证设计。与此同时，项目材料也显示该仓库已经停止维护，且缺少综合报告、时序报告、板级验证结果和资源利用率数据。因此，本文将其定位为 FPGA I2C 接口设计的学习参考、二次移植基础和模块化 HDL 设计案例，而不将其直接视为未经进一步验证即可用于生产环境的最终方案。
...
在现代嵌入式系统和数字逻辑系统中，FPGA 既可以承担高速数据处理、协议转换、实时控制等任务，也经常需要与板级外设进行低速控制通信。相较于高速串行链路或复杂并行总线，I2C 总线具有线路少、连接简单、支持多从设备、外围芯片普遍兼容等特点，因而被广泛用于芯片配置、传感器读取、时钟芯片控制、电源管理和寄存器访问等场景。对于 FPGA 设计而言，I2C 通信逻辑通常不是系统的主功能模块，却是系统启动、外设配置和运行状态管理不可缺少的基础组成部分。

I2C 总线虽然概念上较为简单，但在 HDL 实现中仍涉及较多细节。主控制器需要产生起始条件、停止条件、地址阶段、读写阶段和 ACK/NACK 检测；从控制器需要完成地址匹配、应答响应、数据接收、数据发送和时钟拉伸；当系统需要通过片上总线控制 I2C 外设时，还需要在协议控制器外部增加寄存器映射、FIFO 缓冲、命令接口和状态反馈机制。如果缺乏成熟组件，开发者往往需要在不同项目中重复编写类似逻辑，这不仅降低开发效率，也使验证工作分散且不完整。

`verilog-i2c-master` 项目正是面向这一类需求的 Verilog I2C 接口组件库。根据项目报告，该项目提供了一组可复用的 I2C 主从控制器、片上总线包装层、初始化状态机和测试平台，目标是在 Verilog 设计中快速集成 I2C 通信能力。该项目既包含核心协议模块，也包含 AXI-Lite 和 Wishbone 适配层，还配套了 Python/MyHDL 行为模型与 Icarus Verilog 协同仿真环境。这种组织方式使其不仅可作为工程复用模块，也适合作为本科阶段学习 HDL 模块化设计、总线接口封装和数字系统验证方法的案例。
...
从工程角度看，本文的意义主要体现在三个方面。第一，I2C 控制器是 FPGA 系统中常见而基础的接口组件，通过分析该项目能够帮助理解协议控制器如何从位级时序扩展为可复用的片上总线外设。第二，该项目同时支持 AXI-Lite 和 Wishbone 等总线封装，体现了接口适配和模块复用的设计思想，具有较好的工程教学价值。第三，项目包含 MyHDL 协同仿真测试平台，展示了 HDL 模块验证不应停留在语法编译层面，而应围绕协议行为、数据流和边界情况构造测试激励。

从学习角度看，该项目适合用于理解 FPGA 外设接口组件库的构建方法。一个可复用 HDL 项目通常不应只包含单一 RTL 文件，而应包含清晰的接口定义、参数化配置、上层包装、示例应用和测试环境。`verilog-i2c-master` 的目录结构和模块划分较好地体现了这一点。通过论文方式展开分析，可以帮助读者形成从“协议功能”到“系统集成”再到“验证支撑”的完整认识。

### 1.3 国内外研究与应用概况

I2C 总线由飞利浦公司提出，长期用于低速片间通信。由于其只需要串行数据线 SDA 和串行时钟线 SCL 两条信号线，且支持多个从设备通过地址区分，因此在嵌入式硬件平台中十分常见。在 MCU 应用中，I2C 控制器往往由芯片内部外设提供，开发者通过寄存器或驱动库完成访问；而在 FPGA 应用中，I2C 控制器可能需要根据系统需求自行实现或集成第三方 HDL IP。

在 FPGA 设计实践中，I2C 控制器可分为简单专用型和通用可复用型。简单专用型控制器通常只面向某一颗外设，在状态机中固定写入初始化序列，适合规模较小、需求固定的设计。通用可复用型控制器则提供命令接口、数据接口、状态反馈和参数化配置，可以由上层 CPU、软核处理器或其他逻辑模块动态控制。`verilog-i2c-master` 更接近后者，它不仅实现基础主从协议，还将控制能力封装到 AXI-Lite 和 Wishbone 等片上总线环境中，从而提高项目复用价值。

在验证方法方面，传统 HDL 项目常使用 Verilog testbench 进行仿真，但当协议行为较复杂时，纯 Verilog 激励编写成本较高。Python/MyHDL、cocotb、SystemVerilog/UVM 等方法为 HDL 验证提供了更灵活的测试组织方式。项目报告显示，`verilog-i2c-master` 使用 MyHDL 行为模型和 Icarus Verilog 协同仿真，通过 Python 负责测试激励和总线模型，Verilog 负责被测模块实现。这种方式便于构造较复杂的总线交互场景，也便于在测试脚本中加入断言和数据检查。
...
本文共分为七章。第一章为绪论，介绍研究背景、目的意义、应用概况、研究内容和论文结构。第二章介绍相关技术基础，包括 I2C 总线、Verilog HDL、FPGA 模块化设计、AXI-Lite、Wishbone、AXI Stream 和协同仿真方法。第三章进行系统需求分析，说明项目的功能需求、非功能需求和设计约束。第四章介绍系统总体设计，包括工程结构、模块层次、接口关系和数据流程。第五章进行详细设计与实现分析，重点说明各核心模块的功能和关键逻辑。第六章介绍测试与验证设计，分析测试平台组成、验证方法、已有材料能够支持的结果和不足。第七章为总结与展望，对全文进行归纳并提出后续优化方向。

## 2 相关技术与理论基础

### 2.1 I2C 总线协议概述

I2C 总线是一种同步串行通信总线，通常由串行时钟线 SCL 和串行数据线 SDA 构成。总线上的设备通过开漏方式连接，依靠上拉电阻维持高电平，任一设备均可主动拉低信号线。该特性使多个设备共享同一总线成为可能，同时也要求控制器在输出高电平时并不直接驱动高电平，而是释放总线，由上拉电阻或外部电路使总线回到高电平。

I2C 通信通常由主设备发起。主设备产生 SCL 时钟，并通过 SDA 发送起始条件、从设备地址、读写位、数据字节和停止条件。从设备根据地址判断是否响应，并在每个字节之后返回 ACK 或 NACK。对于读操作，从设备在主设备提供的时钟下发送数据；对于写操作，主设备发送数据，从设备接收并应答。

在 HDL 实现中，I2C 的关键在于正确生成和检测时序。起始条件通常表现为 SCL 为高时 SDA 从高变低，停止条件表现为 SCL 为高时 SDA 从低变高。数据位一般在 SCL 低电平期间变化，在 SCL 高电平期间保持稳定。控制器还需要处理 ACK 检测、重复起始、连续读写、时钟拉伸和总线忙状态等情况。项目报告中指出，`i2c_master.v` 支持读、写、连续写、起始/重复起始、停止等操作，并通过 `missed_ack` 状态输出检测从机未应答；`i2c_slave.v` 支持 I2C 读写事务解析，并能在读取数据尚未准备好时通过保持 SCL 为低实现时钟拉伸。这些功能均与 I2C 协议的典型要求一致。

### 2.2 Verilog HDL 与 FPGA 设计

Verilog HDL 是数字逻辑设计中常用的硬件描述语言，可用于描述组合逻辑、时序逻辑、状态机、模块接口以及层次化电路结构。FPGA 设计通常以 Verilog 或 VHDL 为主要输入，通过综合工具将 RTL 描述映射到目标器件的查找表、触发器、块 RAM、DSP 和 I/O 资源上。
...
模块化设计是 HDL 工程复用的基础。一个复杂接口组件可以拆分为协议核心、数据缓冲、寄存器接口、总线桥接和测试支撑等多个模块。通过清晰的接口边界，不同模块可以独立开发、仿真和复用。参数化设计则允许同一模块在不同场景下通过参数调整位宽、深度、地址范围或功能开关，减少重复代码。
...
AXI-Lite 是 AXI 总线族中的轻量级寄存器访问接口，适合低带宽控制寄存器读写。FPGA 或 SoC 系统中，软核 CPU 或片上控制器可通过 AXI-Lite 访问外设寄存器。项目中的 `i2c_master_axil.v` 将 I2C 主控制器包装为 AXI-Lite 从设备，提供状态、命令、数据和分频等寄存器，使 CPU 可以用寄存器读写方式控制 I2C 通信。
...
根据项目报告，`verilog-i2c-master` 的项目目标是提供一组可复用的 I2C 主从控制器、总线包装层与初始化模块，用于在 Verilog 设计中快速集成 I2C 通信能力。该目标可以理解为三个层次。第一层是协议功能层，即实现 I2C 主模式和从模式基本通信能力。第二层是系统集成层，即将协议核心封装为可由 AXI-Lite、Wishbone 等片上总线访问的模块。第三层是工程支撑层，即提供初始化模板、简单从设备示例和测试平台，降低用户理解、移植和验证成本。
...
`verilog-i2c-master` 项目采用分层架构，可概括为核心层、扩展层、应用层、支撑层和验证层。

核心层由 `i2c_master.v` 和 `i2c_slave.v` 构成，负责实现 I2C 主机和从机基本协议行为。它们直接处理 I2C 时序、地址、读写、应答和状态反馈，是整个项目的基础。
...
这种目录组织方式也体现了设计与验证分离的工程习惯。对于 HDL 项目而言，可综合代码和测试代码在语义上具有明显区别。前者需要经过综合、布局布线并最终映射到 FPGA 资源，后者主要服务于仿真验证，不进入最终硬件。将二者分开放置可以减少误用风险，也便于自动化脚本分别处理 RTL 文件和测试文件。同时，项目没有把所有功能堆叠在单个模块中，而是把主机、从机、包装层、桥接层和示例模块拆分为独立文件，有利于用户按需选取。

从复用角度看，`rtl/` 中每个模块都对应较明确的应用场景。用户如果只需要最基础的 I2C 主机，可以直接集成 `i2c_master.v`；如果已有 AXI-Lite 控制总线，则可以选择 `i2c_master_axil.v`；如果系统使用 Wishbone，则可以选择对应 Wishbone 版本；如果需要外部 MCU 通过 I2C 访问 FPGA 内部寄存器，则可以使用从机桥接模块。这种按功能拆分的组织形式降低了项目集成成本。
...
最后，控制器通过状态信号反馈执行情况。例如，当从设备未应答时，`missed_ack` 可用于提示上层逻辑通信失败。若总线仍处于连续访问状态，相关状态也可反映当前总线是否忙碌或保持控制。
...
项目提供的从机桥接层进一步扩展了 I2C 从设备能力。以 `i2c_slave_axil_master.v` 和 `i2c_slave_wbm.v` 为例，外部 MCU 可以通过 I2C 写入目标地址，然后继续写入数据或发起读操作。模块在内部保存地址指针，并将后续事务转换为 AXI-Lite 或 Wishbone 总线访问。
...
`i2c_master.v` 是项目的核心模块之一，用于实现 I2C 主模式通信。根据项目报告，该模块命令接口支持 `read`、`write`、`write multiple` 和 `stop` 等操作。其关键特性包括支持显式起始和停止，也支持根据总线状态隐式起始；通过 `prescale` 参数进行分频；提供 `missed_ack` 状态输出；内部分为较高层命令状态机和底层 PHY 状态机。

I2C 主控制器的设计难点在于将上层抽象命令可靠转换为总线上的时序变化。上层逻辑希望用“读一个字节”“写多个字节”“发送停止”等命令描述事务，而 I2C 总线实际要求控制器逐位控制 SDA 和 SCL，并在每个字节之后检测或生成 ACK。因此，分层状态机设计是合理的。命令状态机面向事务语义，决定下一步应该发送地址、发送数据、接收数据还是停止；PHY 状态机面向物理时序，负责在正确时刻改变或采样信号。

`prescale` 分频参数用于生成 I2C 时钟。项目报告指出源码中给出公式 `prescale = Fclk / (FI2Cclk * 4)`。这说明模块内部可能将一个 I2C 时钟周期划分为若干阶段，用于控制 SCL 低电平、高电平以及数据建立和保持时间。通过分频参数，用户可以在不同系统时钟频率下生成目标 I2C 速率。
...
`i2c_slave.v` 用于实现 I2C 从模式通信。项目报告指出，该模块支持 I2C 读写事务解析，将写入的字节转换为输出 AXI Stream 数据，将待发送数据从输入 AXI Stream 读取后返回给 I2C 主机，并支持时钟拉伸。此外，模块提供 `busy`、`bus_address`、`bus_addressed`、`bus_active` 等状态信号，以及 `device_address` 和 `device_address_mask` 用于设备地址匹配。
...
`i2c_master_axil.v` 将 I2C 主控制器封装为 AXI-Lite 从设备。根据项目报告，该模块提供 4 个主要寄存器：状态、命令、数据、分频。状态寄存器包含 `busy`、`bus_cont`、`bus_act`、`miss_ack` 以及 FIFO 状态；命令寄存器允许组合 `start`、`read`、`write`、`write multiple` 和 `stop`；数据寄存器既可写入待发送字节，也可读取接收字节；模块还提供可配置的命令 FIFO、写 FIFO 和读 FIFO 深度。

AXI-Lite 包装层的价值在于使 I2C 控制器可以被 CPU 或软核处理器以寄存器方式访问。对于 SoC 风格 FPGA 系统，软件通常通过读写外设寄存器控制硬件模块。如果只有 AXI Stream 命令接口，软件访问并不方便；而 AXI-Lite 包装层提供了标准总线接口，使控制流程可以由软件驱动。

FIFO 的引入有助于提高控制器与软件之间的解耦程度。软件可以连续写入多个命令或数据，控制器按协议时序逐步执行；读回数据也可以先进入读 FIFO，再由软件读取。这样可以降低软件轮询频率对总线时序的影响。对于低速 I2C 总线而言，FIFO 深度不一定需要很大，但可配置 FIFO 提供了更好的适配能力。

### 5.4 Wishbone 主机包装层设计

项目提供 `i2c_master_wbs_8.v` 和 `i2c_master_wbs_16.v` 两个 Wishbone 主机包装层。根据项目报告，`i2c_master_wbs_8.v` 将状态、FIFO 状态、命令地址、命令、数据和分频映射到不同地址，适合基于 Wishbone 的软核系统或开源总线架构。`i2c_master_wbs_16.v` 可以合理理解为采用相近思路，但接口和寄存器宽度适配为 16 位。
...
这种设计使 FPGA 可以作为 I2C 外设挂接到外部控制器上。外部 MCU 只需要通过 I2C 写入地址和数据，就可以访问 FPGA 内部寄存器。这在系统调试、板级控制、状态读取和低速管理场景中非常有用。例如，一个 FPGA 系统可能内部包含多个控制寄存器，但外部主控制器只提供 I2C 接口。通过桥接模块，可以在不增加复杂外部接口的情况下实现控制访问。
...
`i2c_init.v` 是一个模板化初始化状态机。根据项目报告，该模块通过 `init_data` ROM 存储 9 位宽初始化指令，支持单设备初始化和多设备初始化两种模式，可插入延时命令；`start` 拉起后，模块自动按表驱动输出 I2C 主控命令与数据流。

在实际 FPGA 系统中，许多外设需要在上电后写入一系列配置寄存器。例如时钟芯片需要设置频率、输出通道和锁相环参数；图像传感器需要配置分辨率、曝光和数据格式；电源管理芯片需要配置电压轨和时序。如果系统没有嵌入式 CPU，或者希望在 CPU 启动前完成部分硬件初始化，就可以使用硬件初始化状态机。
...
从工程设计角度看，FIFO 的意义不仅在于缓存数据，还在于吸收不同模块之间的速率差异。AXI-Lite 或 Wishbone 侧的控制逻辑可能在系统时钟下快速写入命令，而 I2C 总线受分频参数和协议时序限制，执行速度明显较慢。若没有缓冲机制，上层控制逻辑必须频繁等待底层协议完成，系统吞吐和软件控制体验都会受到影响。通过可配置 FIFO，模块可以在一定范围内连续接收命令或数据，再由协议状态机逐项执行。
...
`prescale` 是 I2C 主控制器中最直接影响通信速率的参数。由于 FPGA 系统时钟通常远高于 I2C 总线速率，控制器需要通过分频产生合适的 SCL 时序。项目报告中给出的公式表明，设计者将 I2C 一个周期划分为多个内部阶段，以便控制数据建立、采样和保持过程。用户在移植模块时，应根据系统时钟频率和目标 I2C 速率重新计算该参数。
...
这种测试组织方式体现了分层验证思想。对于主控制器，可以使用 I2C 存储器模型作为被访问对象，测试不同地址、延迟和命令/数据流。对于从控制器，可以使用 I2C 主机模型驱动 DUT，检查其地址响应、数据接收、数据发送和时钟拉伸行为。对于初始化模块，可以检查其发出的地址与数据序列是否符合预期。对于总线包装层，可以通过 AXI-Lite 或 Wishbone 行为模型模拟软件或片上总线访问。
...
本文基于 `project-report.md` 对 `verilog-i2c-master` 项目进行了本科毕业论文形式的整理和分析。该项目是一个面向 FPGA/HDL 开发的 I2C 接口组件库，提供 I2C 主从控制器、AXI-Lite 和 Wishbone 包装层、I2C 到片上总线桥接模块、初始化状态机、单寄存器从设备示例以及 MyHDL 协同仿真测试平台。

通过需求分析可以看出，该项目面向 FPGA 外设控制、上电初始化、片上总线桥接和寄存器访问等典型场景，功能需求较完整，非功能需求体现出可移植性、可配置性、接口规范性和可验证性。通过总体设计分析可以看出，项目采用分层结构，将核心协议引擎、总线适配、应用示例和测试支撑分离，有利于模块复用和系统集成。通过详细设计分析可以看出，主控制器采用事务级命令状态机与位级 PHY 状态机分离的思路，从控制器通过 AXI Stream 接口与上层逻辑解耦，总线包装层和桥接层则使 I2C 协议能力能够嵌入更复杂的片上系统。
...
第二，接口适配能力较强。项目不仅提供基础 I2C 主从控制器，还提供 AXI-Lite 和 Wishbone 适配层，能够适应不同片上系统环境。

第三，参数化程度较高。项目支持分频、滤波长度、FIFO 深度、数据位宽、地址位宽和设备地址掩码等配置，有利于在不同系统中移植。

第四，验证支撑较完整。项目包含 MyHDL 协同仿真测试平台，测试文件不只是形式化存在，而是围绕主机、从机和初始化模块构造了具体测试场景。

第五，教学价值较明显。项目展示了从基础协议控制器到系统级总线接口封装的完整过程，适合用于学习 HDL 模块化设计和接口组件库构建方法。
...
在文档增强方面，可以补充模块接口时序图、寄存器映射表、典型使用流程和移植指南。对于 AXI-Lite 和 Wishbone 包装层，应给出软件访问示例；对于 `i2c_init.v`，应给出初始化指令表的配置方法和实际外设配置案例。

综上所述，`verilog-i2c-master` 虽然已经停止维护，但其模块结构、接口封装和测试组织仍具有较高参考价值。通过对该项目进行系统分析，可以帮助理解 FPGA I2C 接口组件库的设计方法，也为后续构建更完整、更现代的 HDL 外设接口库提供基础。

## 致谢

在本文整理过程中，项目报告 `project-report.md` 为论文提供了主要事实依据，包括项目目标、模块结构、功能需求、实现内容和测试平台说明。感谢开源项目作者提供 Verilog I2C 接口组件库，使学习者能够通过真实工程案例理解 I2C 协议控制器、片上总线封装和协同仿真测试方法。感谢指导教师在毕业设计选题、论文结构和技术表达方面给予的指导。由于当前材料未包含实际仿真运行日志、综合报告和板级测试结果，本文在相关结论中采用谨慎表述，后续仍需通过实际实验进一步完善。
